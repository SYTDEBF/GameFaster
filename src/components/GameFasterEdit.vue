          ...
          <el-form ref="frForm" :model="fastRecordForm" :rules="addFormRules" label-width="120px"
            ...
            <el-form-item label="规则" prop="ruleId" style="width: 50%">
              <el-select v-model="fastRecordForm.ruleId">
                <el-option v-for="rule in gameObj.rulesList" :key="rule.rulesId" :label="rule.name"
                           :value="rule.rulesId"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="平台" prop="platformId" style="width: 50%">
              <el-select v-model="fastRecordForm.platformId">
                <el-option v-for="pla in gameObj.platformList" :key="pla.platformId" :label="pla.name"
                           :value="pla.platformId"></el-option>
              </el-select>
            </el-form-item>
            ...
            <el-form-item label="备注" prop="remark" style="width: 50%">
              <el-input v-model="fastRecordForm.remark" rows="5" type="textarea">
              </el-input>
            </el-form-item>
            <el-form-item>
              <el-button plain type="primary" @click="send()">立即提交</el-button>
              <el-button plain type="warning" @click="resetForm()">重置</el-button>
            </el-form-item>
          </el-form>
        </el-card>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    async getFrObj() {
      const {data: res} = await this.$http.get('/api/api/fastRecord/list/one/' + this.$route.params.fid)
      this.fastRecordForm = res
      const {data: res1} = await this.$http.get('/api/api/games/list/one/' + this.fastRecordForm.gameId)
      this.gameObj = res1
    ...
    async send() {
      await this.$refs.frForm.validate(async valid => {
        ...
        if (Number(this.hour) < 0 || Number(this.min) < 0 || Number(this.sec) < 0 || Number(this.ms) < 0) {
          ...
          const {data: res} = await this.$http.put('/api/api/fastRecord/edit/user', this.fastRecordForm)
          if (res.flag) {
            this.resetForm()
            this.$router.back()
